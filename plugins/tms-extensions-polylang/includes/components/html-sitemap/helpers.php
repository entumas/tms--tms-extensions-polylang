...
function tepll_html_sitemap_mixed_to_bool( $value, bool $default = false ) : bool {
...
function tepll_html_sitemap_normalize_args( array $args ) : array {
	...
	$out['hide_empty'] = tepll_html_sitemap_mixed_to_bool( $out['hide_empty'], true );
	...
	foreach ( $out['post_types'] as $post_type => $cfg ) :
		...
		$clean[ $post_type ] = array(
			'max_depth'      => isset( $cfg['max_depth'] ) && null !== $cfg['max_depth']
				...
				: null,
			'show_taxonomy'  => tepll_html_sitemap_mixed_to_bool( $cfg['show_taxonomy'], true ),
			'show_posts'     => tepll_html_sitemap_mixed_to_bool( $cfg['show_posts'], true ),
		...
		if ( ! tepll_html_sitemap_taxonomy_applies_to_type( $clean[ $post_type ]['taxonomy'], $post_type ) ) :
			$clean[ $post_type ]['taxonomy'] = 'post' === $post_type
				? 'category'
				: '';
			if ( '' === $clean[ $post_type ]['taxonomy'] || ! tepll_html_sitemap_taxonomy_applies_to_type( $clean[ $post_type ]['taxonomy'], $post_type ) ) :
...
function tepll_html_sitemap_taxonomy_applies_to_type( string $taxonomy, string $post_type ) : bool {
...
function tepll_html_sitemap_section_max_depth( array $cfg, array $args ) : int {
...
function tepll_html_sitemap_current_lang() : string {
...
function tepll_html_sitemap_get_pages_sort( string $page_sort ) : array {
...
function tepll_html_sitemap_get_pages_at( int $parent_id, string $lang, string $page_sort ) : array {
	$sort  = tepll_html_sitemap_get_pages_sort( $page_sort );
	$query = array(
		'parent'      => $parent_id,
		'post_status' => 'publish',
...
// By term (date desc)
function tepll_html_sitemap_posts_for_term( string $post_type, string $taxonomy, int $term_id, string $lang ) : array {
	$cache_key = 'tepll_html_sitemap_posts_for_term_' . md5( wp_json_encode( array( $post_type, $taxonomy, $term_id, $lang ) ) );
...
function tepll_html_sitemap_posts_flat( string $post_type, string $lang ) : array {
	$q = array(
		'post_type'              => $post_type,
		'posts_per_page'         => -1,
		'post_status'            => 'publish',
...
function tepll_html_sitemap_terms_list( string $taxonomy, bool $hide_empty, string $lang ) : array {
	$args = array(
		'taxonomy'   => $taxonomy,
...
function tepll_html_sitemap_render_post_type_section( string $post_type, array $cfg, array $args, string $lang ) : string {
	...
	$section_max = tepll_html_sitemap_section_max_depth( $cfg, $args );
	if ( $section_max < 2 ) return '';

	$taxonomy = $cfg['taxonomy'];
	$show_tax = $cfg['show_taxonomy'] && '' !== $taxonomy && tepll_html_sitemap_taxonomy_applies_to_type( $taxonomy, $post_type );
	...
	if ( $show_tax ) :
		$terms = tepll_html_sitemap_terms_list( $taxonomy, $args['hide_empty'], $lang );
		if ( array() !== $terms ) :
			foreach ( $terms as $term ) :
				...
				if ( $show_pts ) :
					$posts  = tepll_html_sitemap_posts_for_term( $post_type, $taxonomy, (int) $term->term_id, $lang );
	...
	elseif ( $show_pts ) :
		$posts  = tepll_html_sitemap_posts_flat( $post_type, $lang );
		$inner .= tepll_html_sitemap_render_post_links( $posts );
	endif;
...
function tepll_html_sitemap_build_page_li( WP_Post $page, int $depth, array $args, string $lang ) : string {
	...
	if ( $depth < (int) $args['max_depth'] ) :
		$children = tepll_html_sitemap_get_pages_at( (int) $page->ID, $lang, $args['page_sort'] );
		if ( array() !== $children ) :
			$sub = '';
			foreach ( $children as $child ) :
				if ( ! $child instanceof WP_Post ) continue;
				$sub .= tepll_html_sitemap_build_page_li( $child, $depth + 1, $args, $lang );
			endforeach;
			if ( '' !== $sub ) :
...
function tepll_html_sitemap_shortcode_to_bool( string $value, bool $default ) : bool {
...
function tepll_html_sitemap_args_from_shortcode_attributes( array $atts ) : array {
	$args = array(
		'hide_empty' => tepll_html_sitemap_shortcode_to_bool( (string) ( $atts['hide_empty'] ?? '1' ), true ),
	...
	$post_types     = array();
	$blog_on        = tepll_html_sitemap_shortcode_to_bool( (string) ( $atts['blog'] ?? '1' ), true );
	...
	if ( $blog_on ) :
		$blog_cfg = array(
			'show_taxonomy'  => tepll_html_sitemap_shortcode_to_bool( (string) ( $atts['blog_show_taxonomy'] ?? '1' ), true ),
			'show_posts'     => tepll_html_sitemap_shortcode_to_bool( (string) ( $atts['blog_show_posts'] ?? '1' ), true ),
...
function tepll_html_sitemap_get_public_cpt_definitions_for_editor() : array {
...
function tepll_html_sitemap_get_blog_taxonomy_definitions_for_editor() : array {
	$list = array();
...
function tepll_html_sitemap_args_from_block_attributes( array $attrs ) : array {
	$cpt_only  = array();
	...
	foreach ( $cpt_rows as $row ) :
		...
		$cfg = array(
			'show_taxonomy'  => tepll_html_sitemap_mixed_to_bool( $row['show_taxonomy'] ?? true, true ),
			'show_posts'     => tepll_html_sitemap_mixed_to_bool( $row['show_posts'] ?? true, true ),
	...
	$sc = array(
		'hide_empty' => tepll_html_sitemap_mixed_to_bool( $attrs['hide_empty'] ?? true, true )
			? '1'
			: '0',
		...
		'page_sort' => isset( $attrs['page_sort'] )
			...
			: 'menu_order',
		'blog' => tepll_html_sitemap_mixed_to_bool( $attrs['blog'] ?? true, true )
			? '1'
			: '0',
		'blog_max_depth' => isset( $attrs['blog_max_depth'] )
			? (string) $attrs['blog_max_depth']
			: '',
		'blog_show_taxonomy' => tepll_html_sitemap_mixed_to_bool( $attrs['blog_show_taxonomy'] ?? true, true )
			? '1'
			: '0',
		'blog_show_posts' => tepll_html_sitemap_mixed_to_bool( $attrs['blog_show_posts'] ?? true, true )
			? '1'
			: '0',
	...
	$args = tepll_html_sitemap_args_from_shortcode_attributes( $sc );
	$args['wrap'] = false;
	return $args;
}

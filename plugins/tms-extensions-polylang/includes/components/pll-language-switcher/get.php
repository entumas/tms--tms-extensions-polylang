...
if ( ! function_exists( 'language_switcher_render_no_translations_notice_nav' ) ) :
	function language_switcher_render_no_translations_notice_nav( string $display, bool $vertical, array $state_classes, array $block_nav = array() ) : string {
		...
		$bn_other = isset( $block_nav['other_attrs'] ) && is_array( $block_nav['other_attrs'] )
			...
			: array();
		$nav_m    = language_switcher_dropdown_merge_ul_surface_attrs(
			implode( ' ', $root_classes ),
			$bn_class,
			'',
			$bn_style
		);
		$nav_rest = language_switcher_dropdown_build_html_attributes_fragment(
			language_switcher_dropdown_filter_empty_attr_pairs( $bn_other )
		);
...
if ( ! function_exists( 'tepll_language_switcher_get_html' ) ) :
	function tepll_language_switcher_get_html( array $args = array() ) : string {
		...
		if ( empty( $lang_rows ) || ! is_array( $lang_rows ) ) :
			if ( $hide_if_no_translation ) :
				return language_switcher_render_no_translations_notice_nav( $display, $vertical, $state_classes, $block_nav );
			endif;
		...
		if ( 'list' === $display ) :
			...
			$list_a = language_switcher_dropdown_merge_ul_surface_attrs(
				'',
				$clickable_class_extra,
				'',
				$clickable_style_extra
			...
			>
				<ul class="tepll-pll-language-switcher-list">
					<?php foreach ( $languages as $i => $lang ) : ?>
						...
						<li class="<?php echo esc_attr( language_switcher_get_li_class( $lang, (int) $i, $total ) ); ?>">
							<a
								...
								<?php endif; ?>
							>
								<?php echo wp_kses_post( language_switcher_get_inner_html( $lang, $parsed_args, false ) ); ?>
							</a>
						</li>
					<?php endforeach; ?>
				</ul>
		...
		if ( $hide_current ) :
			$toggle_lang = $toggle_lang
				? $toggle_lang
				: language_switcher_get_current_language_pll_row( $r_hide_if_no_translation );
		...
		$dropdown_ul = language_switcher_dropdown_merge_ul_surface_attrs(
		...
		$toggle_el = language_switcher_dropdown_merge_ul_surface_attrs(
		...
		$dd_a = language_switcher_dropdown_merge_ul_surface_attrs(
			'',
			$clickable_class_extra,
			'',
			$clickable_style_extra
		...
		>
			...
			>
				<?php echo wp_kses_post( language_switcher_get_inner_html( $toggle_lang, $parsed_args, true ) ); ?>
			</button>
			<ul
			...
			>
				<?php foreach ( $dropdown_langs as $i => $lang ) : ?>
					<?php if ( ! is_array( $lang ) ) continue; ?>
					<?php $li_class = language_switcher_get_li_class( $lang, (int) $i, count( $dropdown_langs ) ); ?>
...
if ( ! function_exists( 'tepll_language_switcher_print_html' ) ) :
	function tepll_language_switcher_print_html( array $args = array() ) : void {
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo wp_kses( tepll_language_switcher_get_html( $args ), language_switcher_get_kses_allowed_html() );
	}
endif;
